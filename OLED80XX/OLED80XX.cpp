...
{
	for (byte i = 0; i < 8; i++)
		digitalWrite(dataPort[i], data & _BV(i));
}
...
void OLED80XX_Class::ClearLine(unsigned char y)
{
	SmallString(0, y, "                      ", NOM_DIS);
}

void OLED80XX_Class::ClearScreen()
{
	unsigned char i, n;
	for (i = 0; i < 8; i++)
		ClearLine(i);
...
void OLED80XX_Class::Set_Pos(unsigned char x, unsigned char y)
{
	wr_byte_cmd(0xb0 + y);
...
void OLED80XX_Class::SmallString(unsigned char x, unsigned char y, char *chr, char mode)
{
	unsigned char j = 0;
	// x=x*6;
	while ((chr[j] != '\0') && (x < 127))
...
void OLED80XX_Class::BigString(unsigned char x, unsigned char y, char *chr, char mode)
{
	unsigned char j = 0;
	// x=x*8;
	while ((chr[j] != '\0') && (x < 127))
...
void OLED80XX_Class::BigString_len(unsigned char x, unsigned char y, char *chr, char mode, unsigned char len)
{
	unsigned char j = 0;
	// x=x*8;
	while ((chr[j] != '\0') && (x < 127) && (j < len))
...
void OLED80XX_Class::SmallInt(unsigned char x, unsigned char y, long data, byte length, char mode)
{
	unsigned r;
	char *res_str;
	byte neg_flag = 0;
...
void OLED80XX_Class::BigInt(unsigned char x, unsigned char y, long data, byte length, char mode)
{
	unsigned r;
	char *res_str;
	byte neg_flag = 0;
