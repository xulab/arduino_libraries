...
int q_write(uint32_t addr, uint16_t size, char *data, long timeout = -1);
	// void q_read(uint32_t addr, uint16_t size, char *data);
int q_read(uint32_t addr, uint16_t size, char *data, long timeout = -1);
};
